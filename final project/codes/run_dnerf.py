...
def config_parser():
    ...
    # test rendering
    parser.add_argument("--renderonly_render_times",   type=int, default=10,
                        help='how many times render in render_only')

    parser.add_argument("--start_select",   type=int, default=0,
                        help='start select')

    parser.add_argument("--end_select",   type=int, default=-1,
                        help='end select')
...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        images, poses, bds, render_poses, i_test = load_llff_data(args.datadir, args.factor,
                                                                  ...
                                                                  spherify=args.spherify, start_select=args.start_select, end_select=args.end_select)
        times = np.array(load_time(args.datadir, start=args.start_select, end=args.end_select, isForce=True)).astype(np.float32)
        print(times)
        hwf = poses[0,:3,-1] # H,W, Focal Length
        poses = poses[:,:3,:4]
    ...
    print('i_val, i_test', i_val, i_test)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            test_poses_render = torch.Tensor(np.repeat(poses[17][None, ...], args.renderonly_render_times, axis=0)).to(device)
            test_times_render = torch.linspace(0.0, 1.0, steps=args.renderonly_render_times).to(device)

            # print(render_poses.shape)
            # print(render_times.shape)
            # print('.................')
            # print(test_poses_render.shape)
            # print(test_times_render.shape)

            rgbs, _ = render_path(test_poses_render, test_times_render, hwf, args.chunk, render_kwargs_test, gt_imgs=images,
                                  savedir=testsavedir, render_factor=args.render_factor, save_also_gt=True)
            print('Done rendering', testsavedir)
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=args.fps, quality=8)

            return

    # Prepare raybatch tensor if batching random rays
    N_rand = args.N_rand # number of random rays per gradient step
    ...
    N_iters = args.N_iter + 1 # num training iterations
    print('Begin')
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if i >= args.precrop_iters_time:
                ...
                img_i = np.random.choice(i_train[:max_sample]) # randomly sample an image near the reference time at the begning of training
        ...
        if i%args.i_video==0:
            ...
            imageio.mimwrite(moviebase + 'rgb.mp4', to8b(rgbs), fps=args.fps, quality=8)
            imageio.mimwrite(moviebase + 'disp.mp4', to8b(disps / np.max(disps)), fps=args.fps, quality=8)
