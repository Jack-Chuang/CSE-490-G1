...
def config_parser():
    ...
    parser.add_argument("--test_lookat",   type=int, default=0,
                        help='look at pose during testing')
    ...
    parser.add_argument("--test_render_dir",   type=str, default='test_render',
                        help='test_render_dir')    

    parser.add_argument("--fixed_renderpose", type=int, default=1,
                        help='whether to render in a fixed camera pose')  
...
def test():
    ...
    with torch.no_grad():
        ...
        testsavedir = os.path.join(basedir, 'test_render', args.test_render_dir)
        os.makedirs(testsavedir, exist_ok=True)
        

        if args.fixed_renderpose == 1:
            render_poses = torch.Tensor(np.repeat(poses[args.test_lookat][None, ...], args.renderonly_render_times, axis=0)).to(device)
            render_times = torch.linspace(0.0, 1.0, steps=args.renderonly_render_times).to(device)
        ...
        if args.fixed_renderpose == 0:
            psnr_list = []
            for i in range(rgbs.shape[0]):
                mse = torch.mean((torch.from_numpy(rgbs[i]) - images[i]) ** 2)
                psnr_list.append(10 * torch.log10(1 / mse))
            
            print(f'psnr_list: {psnr_list}')
        print('Done rendering', testsavedir)
        imageio.mimwrite(os.path.join(testsavedir, args.video_name), to8b(rgbs), fps=args.fps, quality=8)
